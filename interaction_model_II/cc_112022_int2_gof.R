...
# gof code for interaction model II
...
cat(file = "cc_112022_int2_gof.txt"," 
model {

# Model: cc_112022_int2_gof
#
# psi1 ~ habitat + popden + habitat:popden + re.psi1(area)
# gamma ~ season + elevation + season:elevation + re.gammma(area)
# eps ~ season + elevation + season:elevation + re.eps(area)
...
# gamma ~ season + elevation + season:elevation + re.gamma(area)
...
for (i in 1:nsite){
  for (s in 2:nprimary){
    ...
    + betagamma[1]*season[i,s-1] + betagamma[2]*elevation[i] + betagamma[3]*season[i,s-1]*elevation[i]
    + randomgamma[area[i]]
    
    logit(eps[i,s-1]) <- alphaeps 
    + betaeps[1]*season[i,s-1] + betaeps[2]*elevation[i] + betaeps[3]*season[i,s-1]*elevation[i]
    + randomeps[area[i]]
  }
}
...
cc_112022_int2_gof <- jags(jags.data, inits, params, "cc_112022_int2_gof.txt", n.chains = nc, n.adapt = na, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = TRUE)
...
for (i in 1:cc_112022_int2_gof$mcmc.info$n.samples){
  yrep[[i]] <- cc_112022_int2_gof$sims.list$yrep[i,]
...
for (i in 1:cc_112022_int2_gof$mcmc.info$n.samples){
  p_model[[i]] <- cc_112022_int2_gof$sims.list$pstar[i,]
...
y3drep <- vector(mode = 'list', length = cc_112022_int2_gof$mcmc.info$n.samples)

for (k in 1:cc_112022_int2_gof$mcmc.info$n.samples){
...
p_est <- vector(mode = 'list', length = cc_112022_int2_gof$mcmc.info$n.samples)
...
for (i in 1:cc_112022_int2_gof$mcmc.info$n.samples){
  psi[[i]] <- cc_112022_int2_gof$sims.list$psi[i,,]
  gamma[[i]] <- cc_112022_int2_gof$sims.list$gamma[i,,]
  eps[[i]] <- cc_112022_int2_gof$sims.list$eps[i,,]
...
for (k in 1:cc_112022_int2_gof$mcmc.info$n.samples){
  z[[k]] <- cc_112022_int2_gof$sims.list$z[k,,]
  pstar[[k]] <- p_est[[k]]
