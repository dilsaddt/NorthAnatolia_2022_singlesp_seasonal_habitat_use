...
## Colonization ~ season + popden + elevation + re(area) ----
###############################################
...
col_popden <- array(NA, dim=c(10, 2, cc_072022_full$mcmc.info$n.samples))

for(p in 1:10){ # for popden
  for(i in 1:10){ # for area
    
    col_popden[p,1,] <- plogis(cc_072022_full$sims.list$alphagamma
                               + cc_072022_full$sims.list$betagamma[,2]*popden_d[p]
                               + cc_072022_full$sims.list$betagamma[,3]*0 # mean elev value = 0
                               + cc_072022_full$sims.list$randomgamma[,i]) # summer = 0 so betagamma1 will be gone
    
    col_popden[p,2,] <- plogis(cc_072022_full$sims.list$alphagamma
                               + cc_072022_full$sims.list$betagamma[,1]*1 # winter = 1 
                               + cc_072022_full$sims.list$betagamma[,2]*popden_d[p]
                               + cc_072022_full$sims.list$betagamma[,3]*0 # mean elev value = 0
                               + cc_072022_full$sims.list$randomgamma[,i]) 
  }
}
...
# we don't need to plot different areas, it was just for accounting in the model
col_elev <- array(NA, dim=c(10, 2, cc_072022_full$mcmc.info$n.samples))

for(e in 1:10){ # for elevation
  for(i in 1:10){ # for area
    
    col_elev[e,1,] <- plogis(cc_072022_full$sims.list$alphagamma
                             + cc_072022_full$sims.list$betagamma[,2]*0 # mean popden value = 0
                             + cc_072022_full$sims.list$betagamma[,3]*elevation_d[e]
                             + cc_072022_full$sims.list$randomgamma[,i]) # summer = 0 so betagamma1 will be gone
    
    col_elev[e,2,] <- plogis(cc_072022_full$sims.list$alphagamma
                             + cc_072022_full$sims.list$betagamma[,1]*1 # winter = 1 
                             + cc_072022_full$sims.list$betagamma[,2]*0 # mean popden value = 0
                             + cc_072022_full$sims.list$betagamma[,3]*elevation_d[e]
                             + cc_072022_full$sims.list$randomgamma[,i]) 
...
## Desertion season + popden + elevation + re(area) ----
###############################################
...
#length.out popden X  seasons X mcmc list
# we don't need to plot different areas, it was just for accounting in the model
ext_popden <- array(NA, dim=c(10, 2, cc_072022_full$mcmc.info$n.samples))

for(p in 1:10){ # for popden
  for(i in 1:10){ # for area
    
    ext_popden[p,1,] <- plogis(cc_072022_full$sims.list$alphaeps
                               + cc_072022_full$sims.list$betaeps[,2]*popden_d[p]
                               + cc_072022_full$sims.list$betaeps[,3]*0 # mean elev value = 0
                               + cc_072022_full$sims.list$randomeps[,i]) # summer = 0 so betagamma1 will be gone
    
    ext_popden[p,2,] <- plogis(cc_072022_full$sims.list$alphaeps
                               + cc_072022_full$sims.list$betaeps[,1]*1 # winter = 1 
                               + cc_072022_full$sims.list$betaeps[,2]*popden_d[p]
                               + cc_072022_full$sims.list$betaeps[,3]*0 # mean elev value = 0
                               + cc_072022_full$sims.list$randomeps[,i]) 
  }
}
...
#length.out elevation X  seasons X mcmc list
# we don't need to plot different areas, it was just for accounting in the model
ext_elev <- array(NA, dim=c(10, 2, cc_072022_full$mcmc.info$n.samples))

for(e in 1:10){ # for elevation
  for(i in 1:10){ # for area
    
    ext_elev[e,1,] <- plogis(cc_072022_full$sims.list$alphaeps
                             + cc_072022_full$sims.list$betaeps[,2]*0 # mean popden value = 0
                             + cc_072022_full$sims.list$betaeps[,3]*elevation_d[e]
                             + cc_072022_full$sims.list$randomeps[,i]) # summer = 0 so betagamma1 will be gone
    
    ext_elev[e,2,] <- plogis(cc_072022_full$sims.list$alphaeps
                             + cc_072022_full$sims.list$betaeps[,1]*1 # winter = 1 
                             + cc_072022_full$sims.list$betaeps[,2]*0 # mean popden value = 0
                             + cc_072022_full$sims.list$betaeps[,3]*elevation_d[e]
                             + cc_072022_full$sims.list$randomeps[,i]) 
...
## Detection ~ season + habitat + season:habitat ----
#########################################
...
det <- array(NA, dim = c(2,4,cc_072022_full$mcmc.info$n.samples))

for (i in 1:10){ # for area
  
  # summer, summer = 0 so no betap
  det[1,1,] <- plogis(cc_072022_full$sims.list$alphap[,1] # BL
                      +cc_072022_full$sims.list$randomp[,i]) 
  
  det[1,2,] <- plogis(cc_072022_full$sims.list$alphap[,2] # CF
                      +cc_072022_full$sims.list$randomp[,i]) 
  
  det[1,3,] <- plogis(cc_072022_full$sims.list$alphap[,3] # MF
                      +cc_072022_full$sims.list$randomp[,i]) 
  
  det[1,4,] <- plogis(cc_072022_full$sims.list$alphap[,4] # O
                      +cc_072022_full$sims.list$randomp[,i]) 
  
  # winter, winter = 1
  det[2,1,] <- plogis(cc_072022_full$sims.list$alphap[,1] # BL
                      + cc_072022_full$sims.list$betap[,1]*1
                      +cc_072022_full$sims.list$randomp[,i]) 
  
  det[2,2,] <- plogis(cc_072022_full$sims.list$alphap[,2] # CF
                      + cc_072022_full$sims.list$betap[,2]*1
                      +cc_072022_full$sims.list$randomp[,i]) 
  
  det[2,3,] <- plogis(cc_072022_full$sims.list$alphap[,3] # MF
                      + cc_072022_full$sims.list$betap[,3]*1
                      +cc_072022_full$sims.list$randomp[,i]) 
  
  det[2,4,] <- plogis(cc_072022_full$sims.list$alphap[,4] # O
                      + cc_072022_full$sims.list$betap[,4]*1
                      +cc_072022_full$sims.list$randomp[,i]) 
}
...
## Number of occupied sites - finite sample occupancy ----
###################################################################

plot(cc_072022_full$mean$n.occ)

fin_occu <- data.frame(season=1:22)
fin_occu$pred <- cc_072022_full$mean$n.occ
fin_occu$lower <- cc_072022_full$q2.5$n.occ
fin_occu$upper <- cc_072022_full$q97.5$n.occ
fin_occu$species <- "Roe Deer"
...
prop_occu$pred <- cc_072022_full$mean$n.prop
prop_occu$lower <- cc_072022_full$q2.5$n.prop
prop_occu$upper <- cc_072022_full$q97.5$n.prop
...
# colonization prob w/ human population effect ----------------------------

col_popden$popden_real <- (col_popden$popden*popden_sd) + popden_mean

ggplot(col_popden)+
  #geom_errorbar(aes(areaNr,ymin = lower, ymax = upper, col=season, group=season), size = 1.5, width = 0.2) +
  geom_ribbon(aes(x= popden_real, ymin = lower, ymax = upper, fill=season, group=season),alpha=0.3)+
  geom_point(aes(popden_real, pred, col=season, group=season ), size=3) +
  geom_line(aes(popden_real, pred, col=season, group=season), lwd=1) +
  #scale_color_hue(direction = -1) +
  # geom_text(data=col.prob[1:2,],aes(1:2,pred,label=pred),hjust=0.5, vjust=-3, size=6, fontface="bold")+
  # add_phylopic(roedeer, x=40, y=1, ysize=1,alpha = 0.3, color = "black") +
  ylim(c(0,1)) + 
  theme_bw() + 
...
# colonization prob w/ elevation effect -----------------------------------

col_elev$elev_real <- (col_elev$elevation*elevation_sd) + elevation_mean

ggplot(col_elev)+
  #geom_errorbar(aes(areaNr,ymin = lower, ymax = upper, col=season, group=season), size = 1.5, width = 0.2) +
  geom_ribbon(aes(x= elev_real, ymin = lower, ymax = upper, fill=season, group=season),alpha=0.3)+
  geom_point(aes(elev_real, pred, col=season, group=season ), size=3) +
  geom_line(aes(elev_real, pred, col=season, group=season), lwd=1) +
  #scale_color_hue(direction = -1) +
  # geom_text(data=col.prob[1:2,],aes(1:2,pred,label=pred),hjust=0.5, vjust=-3, size=6, fontface="bold")+
  # add_phylopic(roedeer, x=40, y=1, ysize=1,alpha = 0.3, color = "black") +
  ylim(c(0,1)) + 
  scale_x_continuous(limit=c(700,2200), breaks=seq(700,2200, by=500)) + 
  theme_bw() + 
  theme(panel.grid.minor = element_blank())+
  ylab(expression(paste("Colonization Probabilitiy", " ", (gamma)))) +
  #xlab(expression("Rural human population density "(per~'4'~km^2))) +
...
# desertion prob w/ human population effect -------------------------------

ext_popden$popden_real <- (ext_popden$popden*popden_sd) + popden_mean
...
ggplot(ext_popden)+
  ...
  theme(axis.text= element_text(size=120),
        axis.title.y = element_text(size=140),
        axis.title.x = element_blank(),
        legend.position = "none")

# desertion prob w/ elevation effect --------------------------------------

des_elev$elev_real <- (des_elev$elevation*elevation_sd) + elevation_mean
...
# proportions of used sites -----------------------------------------------

ggplot(propoccu) + 
  geom_ribbon(aes(x=1:22, ymin=lower, ymax=upper), fill="lightgrey") +
  geom_point(aes(1:22, pred), size=10) + 
...
det.prob$habitatAcr <- rep(c("BL","CF","MF","HLU"), times=2)

ggplot(det)+
  geom_errorbar(aes(habitat,ymin = lower, ymax = upper, col=season, group=season), size = 2, width = 0.5, position = position_dodge(width = 1))  +
  #geom_ribbon(aes(x= habitat, ymin = lower, ymax = upper, fill=popden, group=popden),alpha=0.1)+
  scale_fill_continuous(type = "viridis") +
  geom_point(aes(habitat, pred, col=season, group=season ), size=15, position = position_dodge(width = 1))  +
  ...
  guides(colour = guide_legend(title = "Season Transition", title.position="top", title.hjust = 0.5),
