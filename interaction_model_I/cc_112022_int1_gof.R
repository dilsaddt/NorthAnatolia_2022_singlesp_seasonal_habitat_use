...
# gof code for interaction model I
...
cat(file = "cc_112022_int1_gof.txt"," 
model {

# Model: cc_112022_int1_gof
#
# psi1 ~ habitat + popden + habitat:popden + re.psi1(area)
# gamma ~ season + popden + season:popden + re.gammma(area)
# eps ~ season + popden + season:popden + re.eps(area)
...
# gamma ~ season + popden + season:popden + re.gamma(area)
...
for (i in 1:nsite){
  for (s in 2:nprimary){
    ...
    + betagamma[1]*season[i,s-1] + betagamma[2]*popden[i] + betagamma[3]*season[i,s-1]*popden[i]
    + randomgamma[area[i]]
    
    logit(eps[i,s-1]) <- alphaeps 
    + betaeps[1]*season[i,s-1] + betaeps[2]*popden[i] + betaeps[3]*season[i,s-1]*popden[i]
    + randomeps[area[i]]
  }
}
...
cc_112022_int1_gof <- jags(jags.data, inits, params, "cc_112022_int1_gof.txt", n.chains = nc, n.adapt = na, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = TRUE)
...
for (i in 1:cc_112022_int1_gof$mcmc.info$n.samples){
  yrep[[i]] <- cc_112022_int1_gof$sims.list$yrep[i,]
...
for (i in 1:cc_112022_int1_gof$mcmc.info$n.samples){
  p_model[[i]] <- cc_112022_int1_gof$sims.list$pstar[i,]
...
y3drep <- vector(mode = 'list', length = cc_112022_int1_gof$mcmc.info$n.samples)

for (k in 1:cc_112022_int1_gof$mcmc.info$n.samples){
...
p_est <- vector(mode = 'list', length = cc_112022_int1_gof$mcmc.info$n.samples)
...
for (i in 1:cc_112022_int1_gof$mcmc.info$n.samples){
  psi[[i]] <- cc_112022_int1_gof$sims.list$psi[i,,]
  gamma[[i]] <- cc_112022_int1_gof$sims.list$gamma[i,,]
  eps[[i]] <- cc_112022_int1_gof$sims.list$eps[i,,]
...
for (k in 1:cc_112022_int1_gof$mcmc.info$n.samples){
  z[[k]] <- cc_112022_int1_gof$sims.list$z[k,,]
  pstar[[k]] <- p_est[[k]]
